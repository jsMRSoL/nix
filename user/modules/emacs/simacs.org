...
#+begin_src emacs-lisp
  ...
  (defun sp/pgn-to-org ()
    ...
    (let ((tags '("Date" "EndTime" "White" "Black" "Result" "CurrentPosition" "ECO" "ECOUrl" "WhiteElo" "BlackElo" "TimeControl" "Link"))
      ...
      (let* ((utc-date (cdr (assoc "Date" tag-values)))
  	   (utc-time (car (split-string (cdr (assoc "EndTime" tag-values)) " ")))
        ...
        ;; First clear scratch buffer
        (erase-buffer)
        (insert (format "* %s %s\n" utc-date utc-time))
        (insert ":PROPERTIES:\n")
        (sp/pgn-insert-prop "Date" "Date" tag-values)
